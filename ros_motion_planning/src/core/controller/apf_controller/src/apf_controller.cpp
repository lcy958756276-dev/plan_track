...
static constexpr double kApproachWheelLimitNearGoal = 0.1;
static constexpr double kTerminalBrakeTriggerDist = 0.15;
...
APFController::APFController()
  : initialized_(false),
    ...
    hardware_wheel_speed_limit_(0.40),
    tf_(nullptr) {
}
...
void APFController::initialize(std::string name, tf2_ros::Buffer* tf,
  ...
  if (!initialized_) {
    ...
    nh.param("hardware_wheel_speed_limit", hardware_wheel_speed_limit_,
             config_.max_linear_velocity());
    hardware_wheel_speed_limit_ = std::max(0.01, hardware_wheel_speed_limit_);
    ...
    R_INFO << "APF controller initialized! max_linear_velocity="
           << config_.max_linear_velocity()
           << " m/s, hardware_wheel_speed_limit="
           << hardware_wheel_speed_limit_ << " m/s";
  } else {
...
bool APFController::computeVelocityCommands(geometry_msgs::Twist& cmd_vel) {
  ...
  applyWheelSpeedLimit(&cmd_vel, hardware_wheel_speed_limit_, "hardware");
