/**
 * *********************************************************
 *
 * @file: astar_prove_planner.h
 * @brief: Contains the A* with path smoothing planner class
 * @author: Yang Haodong
 * @date: 2023-12-12
 * @version: 1.2
 *
 * Copyright (c) 2024, Yang Haodong.
 * All rights reserved.
 *
 * --------------------------------------------------------
 *
 * ********************************************************
 */
#ifndef RMP_PATH_PLANNER_GRAPH_PLANNER_ASTAR_PROVE_H
#define RMP_PATH_PLANNER_GRAPH_PLANNER_ASTAR_PROVE_H

#include "path_planner/path_planner.h"

namespace rmp::path_planner {
/**
 * @brief Class for objects that plan using the A* algorithm with path smoothing
 */
class AStarProvePathPlanner : public PathPlanner {
public:
  /**
   * @brief Construct a new AStarProve object
   * @param costmap   the environment for path planning
   */
  AStarProvePathPlanner(costmap_2d::Costmap2DROS* costmap_ros);

  /**
   * @brief A* implementation with path smoothing
   * @param start          start node
   * @param goal           goal node
   * @param path           The resulting smoothed path in (x, y, theta)
   * @param expand         containing the node been search during the process
   * @return true if path found, else false
   */
  bool plan(const common::geometry::Point3d& start, const common::geometry::Point3d& goal,
            common::geometry::Points3d* path, common::geometry::Points3d* expand);

private:
  using Node = rmp::common::structure::Node<int>;
  static std::vector<Node> motions_;
};
}  // namespace rmp
#endif
