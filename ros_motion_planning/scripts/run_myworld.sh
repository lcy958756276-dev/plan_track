...
#
# 与 run_debug.sh 的区别:
#   - 使用 gazebo_create 中的自定义地图和世界文件
#   - 使用独立的 Gazebo 节点名、临时 launch 文件、log 子目录
#   - run_debug.sh 保持不变，仍可单独使用
#
# 与 main.sh 的关系:
#   - main.sh: 原版 roslaunch（Gazebo 正常发布 odom TF，用于纯仿真）
#   - run_debug.sh / run_myworld.sh: 设置 /tmp/.use_encoder_odom 标志位，Gazebo 跳过 odom TF
#   - stop_debug.sh / stop_myworld.sh: 清理此标志位

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
WORKSPACE_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
LOG_DIR="$WORKSPACE_DIR/log/myworld"
...
# map_server（提供自定义地图）
MAP_FILE="$WORKSPACE_DIR/../gazebo_create/maps/my_map.yaml"
...
# 使用独立的节点名，避免与 run_debug.sh 冲突
GZ_NAME="__name:=gz_myworld"
rosrun gazebo_ros gzserver "$WORLD_FILE" "$GZ_NAME" \
    >> "$LOG_DIR/run.log" 2>&1 &
PID_GZSERVER=$!
echo "  gazebo server PID=$PID_GZSERVER (节点名: gz_myworld)"
echo "  世界文件: $WORLD_FILE"

# 等待 Gazebo 的 ROS 服务就绪（超时 15 秒）
echo "  等待 Gazebo ROS 服务就绪..."
for i in $(seq 1 15); do
    if rosservice list 2>/dev/null | grep -q "/gz_myworld/set_model_state"; then
...
if ! rosservice list 2>/dev/null | grep -q "/gz_myworld/set_model_state"; then
    echo "  ⚠ 警告：15 秒后 /gz_myworld/set_model_state 仍未就绪"
...
PID_READ=$!
echo "  PID=$PID_READ → log/myworld/serial_bridge.log"
...
PID_ODOM=$!
echo "  PID=$PID_ODOM → log/myworld/encoder_odom.log"
...
PID_SYNC=$!
echo "  PID=$PID_SYNC → log/myworld/gazebo_sync.log"
...
MB_LAUNCH="/tmp/move_base_myworld.launch"
cat > "$MB_LAUNCH" << MBEOF
<launch>
...
echo ""
echo "========================================"
echo "  全部已启动，日志文件在 log/myworld/ 目录下"
echo ""
echo "  ✅ Gazebo + RViz + 实物编码器同步运行（自定义地图）"
...
echo ""
echo "  停止所有进程:   bash $SCRIPT_DIR/stop_myworld.sh"
echo "========================================"
