...
# ── 7.6 启动 path_validator ──
echo "[7.6/8] 启动 path_validator.py (全局路径静态地图碰撞兜底)..."
rosrun encoder_tools path_validator.py \
    > "$LOG_DIR/path_validator.log" 2>&1 &
PID_VALIDATOR=$!
echo "  PID=$PID_VALIDATOR"
...
echo "$PID_VALIDATOR" > "$LOG_DIR/.pid_validator"
