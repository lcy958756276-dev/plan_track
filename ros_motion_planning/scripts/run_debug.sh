...
# my_robot 是工作区根目录下的描述包，而不是 src/ 下的 catkin 包。
# 显式加入搜索路径，确保 Gazebo 和 RViz 都能解析 URDF 中的
# package://my_robot/meshes/*.STL 资源。
export ROS_PACKAGE_PATH="$WORKSPACE_DIR/my_robot:${ROS_PACKAGE_PATH}"
if ! rospack find my_robot > /dev/null 2>&1; then
    echo "错误：ROS 无法找到 my_robot 包，停止启动以避免 RViz/Gazebo 模型加载失败。"
    exit 1
fi

# 远端同步可能丢失 Python 脚本的可执行位。运行前修复，避免 rosrun 找到脚本
# 却拒绝执行，进而导致 /odom 和 map→base_footprint TF 链断开。
ENCODER_SCRIPT_DIR="$WORKSPACE_DIR/src/encoder_tools/scripts"
chmod +x "$ENCODER_SCRIPT_DIR"/*.py 2>/dev/null || true
...
# map_server（提供背景地图）
MAP_FILE="$WORKSPACE_DIR/../gazebo_create/maps/my_four_map.yaml"
MAP_SERVER_SHARE_DIR="$(rospack find map_server 2>/dev/null || true)"
MAP_SERVER_BIN="$MAP_SERVER_SHARE_DIR/../../lib/map_server/map_server"
if [ -x "$MAP_SERVER_BIN" ]; then
    "$MAP_SERVER_BIN" "$MAP_FILE" \
        >> "$LOG_DIR/run.log" 2>&1 &
    PID_MAP=$!
    echo "  map_server PID=$PID_MAP"
else
    PID_MAP=""
    echo "  ⚠ map_server 二进制不可用，/map 不会发布" | tee -a "$LOG_DIR/run.log"
    echo "  请在远端安装/修复 ros-noetic-map-server（期望路径：$MAP_SERVER_BIN）" | tee -a "$LOG_DIR/run.log"
fi
...
# ── 8. 启动 move_base（A* + Double NMPC）──
echo "[8/8] 启动 move_base（A* 全局规划器 + Double NMPC 局部控制器）..."
...
echo "  局部规划器: Double NMPC（0.48s 参考层 + 0.16s 跟踪层）"
...
echo "  ✅ 2D Nav Goal → A* 全局规划 → Double NMPC 路径跟踪"
